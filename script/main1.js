const allStories = [
  ...
  /*function toggleNavbarDisplay() {
    ...
    navbar.style.display = (navbar.style.display === 'none' ? 'flex' : 'none'); // toggle display
}

// Modify the existing showFullView function to hide the navbar
const originalShowFullView = showFullView;
showFullView = (index) => {
    toggleNavbarDisplay();
    originalShowFullView(index);
};

// Modify the existing close button event listener to show the navbar again
closeBtn.addEventListener("click", () => {
    storiesFullView.classList.remove("active");
    toggleNavbarDisplay(); // Toggle navbar visibility when closing the story view
});*/
